...
void LveModel::Builder::loadModel(const std::string &filepath) {
  ...
  for (const auto &shape : shapes) {
    ...
    void LveModel::draw(VkCommandBuffer commandBuffer, VkDescriptorSet globalDescriptorSet, VkPipelineLayout pipelineLayout) {
  ...
  LveModel::LveModel(LveDevice &device, const std::string &filepath, LveDescriptorSetLayout &materialSetLayout, LveDescriptorPool &descriptorPool) : lveDevice{device} {
    std::string warn, err;
    ...
    for(auto& scene : GltfModel.scenes) {
      for(size_t i = 0; i < scene.nodes.size(); i++) {
        ...
        for(auto& GltfPrimitive : GltfModel.meshes[node.mesh].primitives) {
                ...
                std::shared_ptr<Texture> defaultTexture = std::make_shared<Texture>(lveDevice, "textures/white.png");
                ...
                LveDescriptorWriter(materialSetLayout, descriptorPool)
